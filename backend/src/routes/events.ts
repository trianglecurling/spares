...
async function getRegistrationPaymentSummary(paymentOrderId: number | null) {
  ...
  const [paymentTransactions, refunds, refundTotalRows] = await Promise.all([
    ...
    db
      ...
      .where(
        and(
          eq(schema.refunds.payment_order_id, paymentOrderId),
  ...
  const inferredPaidMinor = latestPaymentTransaction?.status === 'succeeded'
    ? latestPaymentTransaction.amount_minor
    : null;
