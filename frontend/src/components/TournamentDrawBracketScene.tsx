...
import {
  ...
  printLoserRouteLabel,
  sheetBadgeGlyph,
} from '../utils/tournamentDrawPrintCard';
import { formatGameScheduleParts } from '../utils/tournamentDrawSchedule';
import {
...
export default function TournamentDrawBracketScene({
  ...
  const metrics = layout.metrics ?? SCREEN_BRACKET_LAYOUT_METRICS;
    ...
    <>
        ...
        ? layout.lanes.map((lane) => {
            ...
            return (
              <div key={lane.eventId}>
                <div
                  className="absolute rounded-md bg-white/50 dark:bg-gray-800/40 border border-gray-200/80 dark:border-gray-700/80"
                  style={{
                    top: lane.top,
                    left: lane.left ?? 0,
                    width: Math.max(0, (lane.width ?? layout.width) - 8),
                    height: lane.height,
                    marginLeft: metrics.backdropLeftInset,
                    borderLeftWidth: 4,
                ...
                >
                  <div
                    className={
                      print
                        ? 'px-2 py-0.5 text-sm font-bold leading-tight tracking-tight'
                        : 'px-3 py-2 text-xl font-bold leading-tight tracking-tight'
                    }
                ...
                {laneGames.length === 0 ? (
                  <div
                    ...
                    style={{
                      left: (lane.left ?? 0) + 12,
                      top: lane.top + metrics.laneHeader,
                      width: metrics.cardW,
      ...
      {!hideVirtualFeederCards
        ? draw.connections.map((c) => {
            ...
            const feederTeamLabel = resolveVirtualFeederCompetitorLabel(
              draw,
              fromG,
              c.place,
              teamsById
            );
            const printVfShowsTeam =
              print &&
              !!feederTeamLabel &&
              printCardShowsParticipants([{ text: feederTeamLabel }], teamsById);
            const body = print ? (
              <>
                <div
                ...
                >
                  {printVfShowsTeam
                    ? printCompetitorDisplayText(feederTeamLabel ?? '', teamsById)
                    : fromG.label}
                </div>
              </>
            ) : (
            ...
            return (
              <div
                ...
                className={
                  print
                    ? 'absolute z-0 rounded-md border border-dashed p-1.5 text-left bg-white/80'
                    : 'absolute z-0 rounded-lg border border-dashed p-2 text-left shadow-sm bg-white/80 dark:bg-gray-900/55'
                }
                style={style}
              >
                {body}
      ...
      {Object.values(draw.games).map((g) => {
        ...
        const printSheetBadge = printSchedule?.sheetName
          ...
          : null;
        const printShowAllRoutes = print && printCardShowsFullRouting(g.slots.length);
        const printLoserRoute = print && !printShowAllRoutes ? printLoserRouteLabel(draw, g) : null;
        const showPrintMeta =
          print && !!(printSchedule?.drawLine || printSheetBadge || printLoserRoute);
        const showParticipants =
          !print || printCardShowsParticipants(competitorSegments, teamsById);
        const showGameLabel = !print || !showParticipants;
        const showRouting = !print || printShowAllRoutes;
          ...
          <>
            ...
            {showParticipants ? (
              <div
                className={
                  print
                    ? 'text-[10px] leading-tight min-w-0 truncate'
                    : 'text-[10px] leading-tight mt-1 min-w-0 line-clamp-2'
                }
              >
                {competitorSegments.map((seg, i) => {
                  ...
                  const slot = si != null ? g.slots[si] : undefined;
                  const registrationId =
                    slot?.sourceType === 'registration' ? slot.registrationId : null;
                  const displayText = print
                    ? printCompetitorDisplayText(seg.text, teamsById, registrationId)
                    : seg.text;
                  return (
                    <Fragment key={i}>
                      {i > 0 ? (
            ...
            {showPrintMeta ? (
              <div className="mt-0.5 flex min-w-0 items-center gap-1 text-[9px] leading-none text-teal-800">
                ...
                {printLoserRoute ? (
                  <span className="shrink-0 tabular-nums text-slate-600">{printLoserRoute}</span>
                ) : null}
              </div>
            ) : schedLine ? (
            ...
            {showRouting && routingLines.length > 0 ? (
              <div
                className={
                  print
                    ? 'mt-0.5 space-y-px text-[8px] font-medium leading-none tabular-nums'
                    : 'text-[10px] font-medium tabular-nums mt-1 pt-0.5 border-t border-gray-100 dark:border-gray-700/80 space-y-0.5'
                }
              >
