...
type HomeSponsorship = HomeData['currentSponsorships'][number];

function uniqueLogoSponsorships(sponsorships: HomeSponsorship[]): HomeSponsorship[] {
  const sorted = [...sponsorships]
...
function premiumSponsorshipsFrom(sponsorships: HomeSponsorship[]): HomeSponsorship[] {
  if (sponsorships.length === 0) return [];
  const highestTierSortOrder = Math.min(...sponsorships.map((s) => s.levelSortOrder));
  return uniqueLogoSponsorships(sponsorships.filter((s) => s.levelSortOrder === highestTierSortOrder));
}
                  ...
                  {displayedSponsorships.map((sponsorship) => (
                    <li key={sponsorship.sponsorshipId}>
                      <SponsorLogoLink
                        ...
                        imgClassName="mx-auto h-auto w-auto max-h-52 max-w-full object-contain sm:max-h-64"
                      />
                    </li>
                  ))}
                </ul>
